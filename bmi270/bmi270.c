...
void BMI270_init(void){
		...
		// initialising
		BMI270_byteWrite(0x7E,0xB6); // cmd soft reset
		systickDelayMs(20);
		BMI270_byteWrite(0x7C,0x00); // advanced power save mode off
		systickDelayMs(10);
		BMI270_byteWrite(0x59,0x00); // prepare load config
		systickDelayMs(10);
		printf("Uploading Config File\n\r");
		BMI270_burstWrite(0x5E, BMI270_CONFIG_FILE_LEN, bmi270_config_file);
		systickDelayMs(30);
		BMI270_byteWrite(0x59,0x01); // complete config load
		printf("Config file uploaded\n\r");
		systickDelayMs(30);
		BMI270_byteRead(0x02, &temp);
		printf("Error reg : 0x%02X\n\r",temp);
		// checking if correct initialization
		BMI270_byteRead(0x21, &temp);
		if(temp & 1<<0){
			printf("Initialization OK\n\r");
		}
		else{
			printf("Initialization ERROR : 0x%02X\n\r",temp);
		}

		// configuring device for low power mode
		BMI270_byteWrite(0x7D,0x0E); // enable gyroscope , accelerometer , temperature sensor
		BMI270_byteWrite(0x40,0xA8);
		BMI270_byteWrite(0x42,0xA9);
		BMI270_byteWrite(0x7C,0x02);

}

void BMI270_readSensorData(int length,char *buffer){
	BMI270_burstRead(0x0C, length, buffer);
}
